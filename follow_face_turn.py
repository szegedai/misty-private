...
def start_face_following_skill(calibration = False):
    ...
    print("start_face_following_skill STARTED")
    ...
    while True:
        ...
        if (seconds_since_last_detection >= 4 or searching_for_face) and not turn_in_progress:
            ...
            if not "key_phrase_turn" in misty.active_event_registrations:
                ...
                misty.StartKeyPhraseRecognition(captureSpeech = False)
                misty.RegisterEvent("key_phrase_turn", Events.SourceTrackDataMessage, callback_function = sound_callback, debounce = 100, keep_alive = True)
            if not "sound" in misty.active_event_registrations:
                misty.RegisterEvent("sound", Events.KeyPhraseRecognized, callback_function = key_phrase_turn_callback, debounce = 10, keep_alive = False)
            searching_for_face = True
            look_side_to_side()
            time.sleep(6.5)
...
def look_at(heading, robot_yaw_at_start, head_yaw_at_start):
    ...
    while (abs(robot_yaw - global_heading) >= 10):
        ...
        if (datetime.now(timezone.utc) - look_at_start_time).total_seconds() > 7:
...
def face_rec_callback(data):
    ...
    if not "voice_cap" in misty.active_event_registrations and not "key_phrase_recognized" in misty.active_event_registrations:
        start_listening()
